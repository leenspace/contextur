...
export function registerReviewCommand(program: Command): void {
  ...
  program
    ...
    .action(async (opts: ReviewIntakeOptions) => {
      ...
      const scopedFromOverride = pathOverride.length > 0 ? scopeFilesByPaths(kept, pathOverride) : kept;
...
interface BuildNonInteractiveReviewCommandOpts {
  baseRef: string;
  reviewers: string[];
  pathFilters: string[];
  focus?: string | undefined;
...
export function buildNonInteractiveReviewCommand(opts: BuildNonInteractiveReviewCommandOpts): string {
...
function groupPaths(files: string[]): PathGroup[] {
...
function preferredPathPrefix(path: string): string {
  ...
  if ((parts[0] === "packages" || parts[0] === "apps" || parts[0] === "src") && parts.length >= 2) {
