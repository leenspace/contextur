---
name: contextur-update
description: Re-scan the repository and refresh Contextur reviewer repo-specific rules without drift.
---
...
If the markers don't exist (reviewer was never personalized), create a `## Repo-specific rules` section at the end using the same format as `contextur-init`.
...
Re-read AGENTS.md. If the `## Architecture` section (originally generated by `contextur-init`) is out of date, update it. Preserve all hand-written sections.
